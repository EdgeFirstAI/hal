...
# four dynamic objects changes where .profraw files originate, and the
...
fail=0
for crate in tensor codec image decoder; do
...
# The four extension cdylibs are excluded from instrumentation on purpose --
...
[ "$fail" -eq 0 ] && echo "✓ coverage attributes to all crates after the split"
exit "$fail"
