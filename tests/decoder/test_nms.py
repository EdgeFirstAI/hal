...
for iou in range(0, 100):
    for score in range(0, 100):
        ...
        indices = tf.image.non_max_suppression(
            boxes, scores, max_output_size=100000, iou_threshold=iou_threshold, score_threshold=score_threshold)
        tf_boxes, tf_scores, tf_classes = boxes[indices], scores[indices], classes[indices]
