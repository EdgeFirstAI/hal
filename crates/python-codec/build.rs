...
/// Single-tensor-home, Python side (task P2): `_codec` links
...
fn link_tensor(target_os: &str) {
    ...
    // (`edgefirst/codec/_codec...` vs. `edgefirst/tensor/_tensor...`), so
    // the relative step has to cross into the sibling package directory
    // rather than stay in the same one. RUNPATH is not transitive, so
