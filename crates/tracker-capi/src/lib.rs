...
//! The C API for `edgefirst-tracker`: ByteTrack multi-object tracking.
//!
...
//! `edgefirst/decoder.h`, and never links `libedgefirst_decoder`.

pub mod bytetrack;

/// ABI version of this library's C surface.
#[no_mangle]
pub extern "C" fn ef_tracker_abi_version() -> u32 {
    1
}
...
mod tests {
    fn header_text() -> String {
        ...
        .expect("cbindgen wrote include/edgefirst/tracker.h")
    }
    ...
    fn cc_syntax_check(src: &str) -> Option<std::process::Output> {
        ...
        match std::process::Command::new(&cc)
            .args([
                ...
                "-I",
                concat!(env!("CARGO_MANIFEST_DIR"), "/../decoder-capi/include"),
                "-I",
    ...
    fn the_generated_header_exists_and_declares_the_abi_probe() {
        assert!(header_text().contains("ef_tracker_abi_version"));
    }
    ...
    fn the_header_carries_none_of_the_monoliths_symbols() {
        assert!(
            ...
            "tracker.h must not carry the monolith's hal_* symbols"
        );
    }

    #[test]
    fn tracker_h_includes_the_detect_header() {
        assert!(
            header_text().contains(r#"#include "edgefirst/detect.h""#),
            "tracker.h must include edgefirst/detect.h"
        );
    }
    ...
    fn no_rust_type_name_leaks_into_the_c_api() {
        ...
        for rust_name in [
            ...
            assert!(
                ...
                "the Rust type name `{rust_name}` leaked into tracker.h; add it to \
    ...
    fn the_header_survives_being_included_twice() {
        ...
        assert!(
            out.status.success(),
            "tracker.h failed to compile when included twice:\n{}",
            String::from_utf8_lossy(&out.stderr)
        );
    }

    #[test]
    fn the_shipped_artifact_is_libedgefirst_tracker_not_capi() {
        ...
        let found: Vec<String> = std::fs::read_dir(&dir)
            .map(|rd| {
                rd.filter_map(|e| e.ok())
                    ...
                    .filter(|n| n.starts_with("libedgefirst_tracker"))
                    .collect()
            })
            .unwrap_or_default();
        if found.is_empty() {
        ...
        assert!(
            ...
            "must ship as libedgefirst_tracker.*, never ..._capi; found {found:?}. \
