...
use crate::{
    error::{Error, Result},
    BufferIdentity, DType, IdentityKind, PixelFormat,
};
...
/// Choose an AHardwareBuffer's identity from the parts the FFI supplies.
///
/// Split out of `ahardwarebuffer::buffer_identity` so the *decision* is
/// testable on every host: that module is `cfg(target_os = "android")`, so
/// on-device is the only place its code runs, and no device is available in
/// CI. Compiling proves it builds, not that it discriminates between
/// buffers — and the pre-31 branch passing a constant instead of `ptr`
/// would collapse every pre-31 buffer to one identity, which is exactly
/// the failure the derived-identity work exists to prevent.
pub(crate) fn identity_from_parts(id: Option<u64>, ptr: u64) -> BufferIdentity {
    match id {
        // API 31+: the system's own id, stable across every wrap of the
        // same allocation and agreed on by independently-linked copies of
        // this crate.
        Some(id) => BufferIdentity::derived(IdentityKind::AHardwareBufferId, id),
        // Pre-31: no system id exists. The pointer is unique among live
        // buffers in this process, which is all a per-process cache needs.
        None => BufferIdentity::derived(IdentityKind::AHardwareBufferPtr, ptr),
    }
}
...
mod tests {
    ...
    #[test]
    fn pre_api_31_buffers_do_not_share_an_identity() {
        // The defect this whole task guards against, in the one path with
        // no device to catch it: a constant key would collapse every
        // pre-31 buffer to one identity.
        assert_ne!(
            identity_from_parts(None, 0x1000).id(),
            identity_from_parts(None, 0x2000).id(),
            "pre-31 buffers must be distinguished by their pointer"
        );
    }

    #[test]
    fn the_system_id_is_preferred_over_the_pointer_when_present() {
        let a = identity_from_parts(Some(7), 0xDEAD);
        assert_eq!(a.kind(), IdentityKind::AHardwareBufferId);
        // Same pointer, different system id => different identity, which
        // proves the pointer is not what got used.
        assert_ne!(a.id(), identity_from_parts(Some(8), 0xDEAD).id());
    }
