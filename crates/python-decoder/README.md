...
pixel-space coordinates, TFLite exports report `[0, 1]`, and CoreML exports
resolve pixel-space too, the same as ONNX. `"other"` is accepted but
refused by inference rather than defaulted — that convention follows the
exporter, is not derivable from shapes, and guessing it scales every box by
the input size. Supported dtype strings are `"int8"`, `"uint8"`, `"int16"`,
