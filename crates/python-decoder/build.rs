...
/// Single-tensor-home, Python side (task P2): `_decoder` links
...
fn link_tensor(target_os: &str) {
    ...
    // (`edgefirst/decoder/_decoder...` vs. `edgefirst/tensor/_tensor...`),
    ...
    // whatever `os.add_dll_directory()` registered, never `PATH`. `_decoder.pyd`
    // lives in `edgefirst/decoder/` while `edgefirst_tensor.dll` ships in
    // `edgefirst/tensor/`, so the cross-directory step happens at import
    // time instead: `python/edgefirst/decoder/__init__.py` registers
    // `edgefirst/tensor/` before importing `._decoder`.
    if target_os == "macos" {
