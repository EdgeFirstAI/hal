...
fn main() {
    ...
    cbindgen::Builder::new()
        ...
        .write_to_file(output_dir.join("image.h"));
    ...
    if target_os == "linux" {
        ...
        println!("cargo:rustc-cdylib-link-arg=-Wl,-soname,libedgefirst_image.so.{major}");
    }
    ...
    // functions has no reason to) links this .so without also linking
    ...
    // absolute path would be wrong for an installed copy. See task 9's
    // report ("Bug 3") for the full investigation.
    if target_os == "linux" {
...
fn link_tensor_cdylib(target_os: &str, tensor_lib_dir: &Path) {
    if target_os == "windows" {
        ...
        // rustc-link-arg (not rustc-link-lib, not cdylib-link-arg): cargo
        // emits rustc-link-lib *before* the crate's rlibs, so aarch64 bfd
        // --as-needed drops -ledgefirst_tensor. Trailing link-args land
        // after those rlibs; --no-as-needed keeps the cdylib even if the
        // test binary's own objects do not mention ef_tensor_*.
        println!("cargo:rustc-link-arg=-Wl,--no-as-needed");
        println!("cargo:rustc-link-arg=-ledgefirst_tensor");
