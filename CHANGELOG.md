...
- `[tool.ruff] required-version` sets a `>=0.16.7` floor. ruff 0.15.8 reports five `F403`s on the `edgefirst.*` star re-exports that 0.16.7 does not, so a stale local ruff failed a gate `main` passes; the floor names the version mismatch instead of the symptom.
