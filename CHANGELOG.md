...
- **Documentation review for 0.30.0.** Every claim below was checked against the code, not just re-read:
  ...
  - `ARCHITECTURE.md`'s tracing catalog gained the `"windows"` `platform` value, the `"d3d11"` `image.gl_init` backend, the `tensor.map` `memory` values, `python.convert_with_fence` (added this release) alongside `convert_deferred` / `flush` / `tile_one`, and the fused `codec.decode_jpeg.write_*` family from 0.28.2. Its source-tree listing was missing `tensor-abi/`, `tensor-ffi/`, `decoder-abi/` and `python-tracker/`.
