...
- Quick lints the aarch64 target again. `skip-cross-clippy` was switched on because the shared per-target pass took a fixed argument list and hal's six PyO3 crates cannot cross-compile without an interpreter for the target — so the escape hatch cost hal aarch64 clippy coverage entirely, on a repository whose products are aarch64. The pass now takes the same exclusions the host pass uses, and runs check-only without the `opencv` feature, since opencv-sys probes for a target-native OpenCV that is not installed.

- Quick also runs `actionlint` and the organisation's SHA-pin policy over `.github/`, which arrives with the shared workflows rather than being configured here. The 21 findings it reported are fixed rather than suppressed: unquoted `$GITHUB_STEP_SUMMARY` / `$GITHUB_ENV` / `$GITHUB_OUTPUT` redirects, `export X=$(...)` masking the command's exit status, `for x in $(find ...)` splitting on whitespace in a path, and `sha256sum *` reading a leading-dash filename as an option. The maturin argument lists became arrays, so the expansion is quoted and the `--sdist` filtering is an explicit comparison rather than a substring removal, and the matrix value reaches the script through `env` rather than being interpolated into it. `read -ra` still splits on whitespace without interpreting quotes, which is what the unquoted expansion did too — every value in that matrix is a bare flag, and the step records what to do if one ever is not. One `xargs -0 sh -c '...'` keeps its single quotes with a scoped `shellcheck disable` and the reason, because `$1` there belongs to the inner shell.

- `release.yml` keeps the packaged `.crate` files as artifacts. `publish.yml` runs `cargo publish --no-verify`, and skipping the verification compile on the tag path is only defensible because packaging was exercised on a branch a human reviewed — these are the record of exactly what that packaging produced. `cargo publish` packages from source and cannot consume them, so they are provenance rather than an input.

- The wheel build uses the shared `maturin-build` action for installing maturin, running the builds and checking each wheel's version, instead of hal's own copy of each. The matrix, the ANGLE fetch, the `lld` install, the per-package staging and the two post-build assertions stay here, because they are hal's: an action composes into the job hal already has, where a reusable workflow would have needed a hook for each of them. `--sdist` moved from the lane arguments to `tracker`'s entry in the build list, which is where it belongs — it is per package *and* per lane, and expressing only half of that in the lane arguments is what made the old loop strip it by substring.

  Provenance is still generated by hal rather than by the action (`attest: false`), because the action attests as soon as it has built and these wheels are not shippable until the layout and Mach-O checks have passed. Provenance for a wheel an assertion is about to reject is provenance for something nobody can install.
...
- **CoreML as a recognized model container for schema inference.** `infer_ultralytics_schema` now accepts a CoreML (`.mlpackage` / `.mlmodelc`) no-NMS export and resolves its box coordinates as pixel-space, matching ONNX — established by source-tracing the Ultralytics CoreML exporter and confirmed by the same runtime magnitude measurement that pinned ONNX and TFLite. A `nms=True` CoreML export produces Apple's own NMS-pipeline artifact instead, which is refused rather than resolved. Available from Rust (`ModelSource::CoreMl`), Python (`source="coreml"`), and C (`ef_infer_signals_new(3)`).
...
- The Makefile runs this repo's helper scripts through a probed `$(PYTHON)` rather than a hard-coded `python3`. Git Bash on a Windows dev box often has only `python`, and `python3` there may be the Microsoft Store stub, which exits 49 with "Python was not found" instead of failing to resolve — so the probe runs each candidate rather than trusting `command -v`, the same rule `scripts/package-capi.sh` already applied. This reaches `make wheel`, `make sbom` and `make package`, which previously created their artifacts and then failed on the check that follows.
...
- **A board run that executed no tests is reported as `NO-TESTS` with a non-zero exit, not `PASS`.** `FILTER` is handed to each test binary as a test-*name* filter, so a filter matching the source file rather than the test function inside it leaves every binary printing "running 0 tests" and exiting zero, which counted as a pass. The skip check beside it cannot catch that: it greps each log for `SKIPPED`, and a binary that ran nothing has nothing to skip. A binary running no test now reports it, and a host where every binary ran nothing fails, naming the filter that matched nothing.

- **NOTICE names every third-party project requiring attribution, up from 234 of 246.** Eight were absent because NOTICE predated the dependency bumps introducing them. `libloading` (ISC) and `zlib-rs` (Zlib) were dropped by an attribution set covering only Apache-2.0, the BSD family and MIT. `dma-heap` was dropped because it sets `license-file` rather than `license`, so CycloneDX emits `{"name": "Unknown", "text": ...}` for it while the extractor read only `license.id` and SPDX expressions. The extractor now reads `license.name` when it is a recognised identifier and otherwise identifies the licence from its embedded text; a licence it cannot place is attributed as `Unclassified` rather than discarded. A free-form name such as `MIT License` no longer counts as identification, which would skip the text that can place it.

- **`make sbom` resolves the same dependency graph everywhere.** The shared policy script merges every `*.cdx.json` under the tree, and those are gitignored build residue, so a working tree carrying scans from an earlier graph produced a different answer than a fresh checkout — 360 components against 237, including versions no lockfile pins. Scans are now written under a run-unique name and only those are merged. The sweep itself moved to the shared script (EdgeFirstAI/.github), which resolves a graph per shipped target and per workspace-excluded C-API manifest, so `make sbom`, the `NOTICE attribution` job and the SBOM attached to a release compute one graph from one implementation. The published release SBOM gains the `windows-*` crates and the five `-capi` leaves, which a host-only workspace scan cannot reach. Targets are enumerated rather than using cargo-cyclonedx's `all`, which disables platform filtering and pulls in `[target."cfg(...)"]` blocks for cfgs no build sets.

- **A component requiring attribution but missing from NOTICE fails `make sbom`, and a `NOTICE attribution` job runs the same check on every pull request touching code.** The gate asks `generate_notice.py --check` what the generator would write rather than reading the shared validator's warning, which covers every component regardless of licence while the generator omits those asking for no attribution — gating on the warning would fail on a component NOTICE is correct to omit, with no way to clear it. Matching on name and version also catches a dependency bump that left NOTICE naming the old one. The job sits on the Quick trigger rather than the Full tier, which runs only when the `ci:full` label is applied.

- **`make test-python` builds a wheel on macOS again (#199).** The target installs through `pip install crates/python-*/`, which drives maturin's PEP 517 backend — the one maturin call site in the repo that takes no CLI flags of its own, so it was also the only one whose wheel-repair step still ran. On macOS that step fails with `Cannot repair wheel, because required library @rpath/libedgefirst_tensor.0.dylib could not be located`. The library is not missing: `crates/python-tensor/build.rs` stages it into `python-source` and the extension carries `-rpath,@loader_path`, but maturin's repair resolves `@rpath/...` against link-time search paths only and cannot see what is about to be packaged, so it declares an already self-contained wheel unrepairable. The loop now passes `MATURIN_PEP517_ARGS="--auditwheel skip"`, matching `build-python`, `wheel` and release.yml's wheel job. Linux never hit it because maturin skips auditwheel entirely under the `--compatibility off` pip's invocation carries; the macOS dylib-repair path has no such opt-out. The install loop is now its own `make install-python` target, narrowable with `PYTHON_PACKAGES=tensor`, and hal-full.yml's macOS job runs it plus an import — so the source-directory install path is gated nightly for the first time. Python was already tested nightly (the Linux job runs the full suite, the macOS job the `-m gpu` subset), but both install from `maturin build` output; `pip install <dir>/` ran in no lane, which is why this survived.

- **Release builds no longer produce Mach-O images dyld refuses to load (#200).** `[profile.release] strip = true` makes rustc strip Apple binaries with its own in-process Mach-O writer rather than shelling out to Apple's `strip(1)`, and that writer places the string table immediately after the indirect symbol table with no padding. Indirect symbol entries are four bytes, so any binary whose `nindirectsyms` is odd gets a 4-byte-aligned `stroff`, and dyld on macOS 26+ rejects the image: `ImportError: dlopen(..._tensor.cpython-314-darwin.so, 0x0002): mis-aligned LINKEDIT string pool, fileOffset=0x0019070C`. The parity is a coin flip per binary, so this was latent across every release Mach-O the workspace produces, not a property of one crate — at 0.31.0 `_tensor` (435) and `_decoder` (493) were odd and unloadable while the other three extensions were even and fine by luck. `maturin build --release` is release.yml's wheel path, so it reached published macOS wheels. `.cargo/config.toml` now hands the strip back to Apple's linker on both macOS triples (`-C strip=none -C link-arg=-Wl,-x`), which pads correctly; `-x` is strictly less destructive than the `strip=symbols` the profile asked for, and costs ~6% of artifact size rather than the ~39% of dropping the strip altogether. The iOS triples carry the same two flags — same Mach-O, same rustc writer — measured at +7% on a real `aarch64-apple-ios` link (1.26 MB → 1.35 MB); this repo builds only rlibs for iOS today, so they are inert until something links, and `mobile-sdk` needs the same pair on its side. Android deliberately does not: it is ELF, which has no LINKEDIT segment and no string-pool alignment rule, and `-Wl,-x` means `--discard-all` to lld rather than what rustc's strip does — measured on a real `aarch64-linux-android` link, rustc's strip drops `.symtab` outright for 1.32 MB while the flags put every global symbol back for 3.30 MB, a 2.5× cost to fix a defect ELF cannot have. Debug backtraces are unaffected (function names and `file:line` both verified). `scripts/check_macho_alignment.py` is the regression gate — pure file parsing, so it is meaningful on any runner — wired into `make wheel`, `make package`, a new `make check-macho`, release.yml's wheel job, and `tests/packaging/test_macho_alignment.py`.

- **The GL float convert route now enters `image.convert.gl` and records `src_feed`/`dst_feed` (#185).** F16/F32 destinations returned before the convert span was opened, so `feed_float_src` recorded against a span that did not declare the fields and `tracing` dropped them. The span is entered on the float dispatch with the same field set as the u8 engine, and recording sites use that handle.

- **The Windows ANGLE D3D11 WARP GL test job is a required check (#186).** The step was `continue-on-error: true`, so a failed nextest run still showed a green PR check. Failures now fail the job. Coverage LCOV still uploads on failure.

- **Dynamic `TensorDyn` is a cache-only lens over `libedgefirst_tensor` (0.32.0 Stages C–E).** Multiplane is answered with `ef_tensor_is_multiplane` / `ef_tensor_chroma`; CUDA registration attaches through `ef_tensor_cuda_attach` (frozen `ef_client_state`, no new `repr(C)` struct and no `edgefirst_tensor_v3`). `from_raw` after `from_planes` or CUDA attach no longer drops that state. `ef_tensor_wrap_host` refusals set `errno` and `ef_tensor_last_error_class` like `wrap_pbo`. Capsule keepalive / `PboOpsVtable` layout is unchanged.

- **The nightly mutation-testing roll-up failed on its own report rather than on a result.** It listed survivors with `cat shards/mutants-shard-*/mutants.out/missed.txt | head -60` under `set -euo pipefail`: past sixty survivors `head` closes the pipe, `cat` takes `SIGPIPE`, `pipefail` propagates the non-zero status and `set -e` aborts the step — after the summary has already been written, so the run showed a red X beside a complete report. The two runs with fourteen survivors passed; the run with a hundred and sixty-eight failed. The roll-up now reads each shard's `outcomes.json` through `.github/scripts/mutants_summary.py` with no pipeline to truncate, and reports the night as a mermaid pie, a table saying what each outcome means, files ranked by survival rate, and every survivor grouped under its file and function with the diff no test noticed. The job fails when a mutant survives, which is what the lane is for, and no longer fails when nothing is wrong.

- **110 mutants that survived the decoder's suite are now caught: mutations the whole suite passed with.** The largest single group was `to_xyxy_float`, twenty of them. Both `XYXY` and `XYWH` overrode `ndarray_to_xyxy_float` with a hand-inlined copy of their own `to_xyxy_float` body, and that override is what the float postprocessors call, so the trait default was the only route to the slice method and nothing took it — the mutations survived in a conversion neither the tests nor production reached. The overrides are gone, so there is one implementation per type, the postprocessors reach it through the trait default, and it is tested directly. The class-aware and `_extra` NMS variants had `max_det` truncation tests but no suppression test, so a loop that stopped at the first survivor, suppressed across classes instead of within one, or marked a suppressed box with `1.0` instead of `-1.0` all went unnoticed. The score threshold is a floor and no test sat a score exactly on it. `Quantization::try_from` asserted only the error variant, which the catch-all arm produces too, so guard mutations that skipped the intended match arm still passed. `postprocess_modelpack_split_float` had no direct test at all. Of the fifty-eight still surviving, none is a test gap: thirty-three are in `#[cfg(target_arch = "aarch64")]` code an x86_64 build never compiles, and the rest are equivalent mutants — `identity()` against a `Default` that is the same value, `survivors += 1` guarded by a `.take(cap)` that makes the early break unobservable, capacity arithmetic that only sizes a `Vec`, and layout dispatch whose two paths agree by design.

- **Mutation testing never reached the decoder's NEON code, and reported it as untested rather than unreached.** cargo-mutants mutates source, not the compiled crate, so it happily mutated `arg_max_i8`, `fast_arg_max`'s dispatch, the column-max kernels and all of `per_scale/kernels` on an x86_64 runner that never builds them; thirty-three mutants came back surviving when no test that ran there could have caught them. Every shard now runs on `ubuntu-24.04-arm` as well, and the roll-up merges the sweeps: a mutant is caught when any architecture's tests caught it and survives only when every architecture that tested it let it through, so neither lane reports the other's `#[cfg]`-gated code as a test gap. Survivors carry the architectures that ran them.
...
- **Windows zero-copy tensors.** `TensorMemory::DmaBuf` on Windows is an `ID3D11Texture2D` on a HAL-owned D3D11 device that ANGLE renders with (`crates/tensor/src/d3d11/`). `ImageProcessor::create_image` allocates textures first; the GL engine imports them as EGLImages (`EGL_ANGLE_image_d3d11_texture`) re-attached per frame (persistent bindings sample stale content over a D3D11 EGLImage); `map()` goes through a staging copy whose pitch is the tensor's row stride; F16 and F32 planar, interleaved and RGBA float destinations render zero-copy. CUDA reads textures as external memory (`cuda_map`, one device copy per map; `cuda_map_mut` writes back). Every convert records a D3D11 fence value on its destination; `convert_with_fence` returns an event handle on Windows; `gpu_completion` hands GPU consumers the fence handle and value; `gpu_write_value` is the value alone, for a consumer that already holds the process fence. Textures are shared through NT handles (`d3d11_shared_handle`, `from_d3d11_shared_handle`), across processes through `ef_tensor_export` / `ef_tensor_import` (handles duplicated on import), and across independently linked packages through the new `D3D11_TEXTURE` protocol kind; copies of the tensor crate in one process share one device. A host that already owns a D3D11 device can install it with `edgefirst_tensor::d3d11::use_external_device` (`ef_d3d11_use_external_device`, `edgefirst.tensor.d3d11_use_external_device`) before anything creates the HAL's, so HAL tensors are allocated on the caller's device; `ef_d3d11_device` hands back whichever device is in force. The full surface exists in Rust, the C API (`ef_d3d11_*`, `ef_tensor_d3d11_*`, `ef_tensor_gpu_completion`, `ef_tensor_gpu_write_value`, `ef_tensor_set_gpu_write`, `ef_tensor_from_d3d11_*`, `ef_tensor_try_map`, `ef_tensor_cuda_map_mut`, `ef_image_processor_convert_fence_handle`) and Python. `EDGEFIRST_D3D11_ADAPTER` joins `EDGEFIRST_ANGLE_ADAPTER` (both valid). Measured on an RTX 3070 and on WARP (internal design record).
- `Tensor::try_map` / `try_map_with` (non-blocking map) and `cuda_map_mut` on every platform, in Rust, C and Python.
- The tensor-capi and image-capi C tests (`tests/c/*.c`) now compile, link and run on Windows from any shell. The harness locates MSVC itself (vswhere + `VsDevCmd.bat`, or the current developer shell), compiles with `/W4 /WX`, and links `target/debug/edgefirst_tensor.lib` (image-capi: `edgefirst_image.lib` plus the tensor DLL's import library). Previously they looked for `libedgefirst_tensor.a` and skipped on every Windows host. Discovery happens once per test process and each process compiles into its own scratch directory, so the programs build at the default test parallelism; the libraries are looked for in the target directory the test binary itself was built into, then `CARGO_TARGET_DIR`, the workspace `target/` and the leaf's own. Every skip names its reason -- the command that failed and what it printed, the directories searched, or the stale artifact (a stale copy in one candidate directory is passed over for a fresh one in another) -- and the export checks (`the_dynamic_symbol_table_carries_only_ef_symbols`, `the_two_libraries_export_no_symbol_in_common`, the shipped-artifact name) read `dumpbin /exports` and the MSVC artifact names on Windows instead of skipping. A CPU map of a D3D11 texture tensor now queues a GPU-side wait on the write `set_gpu_write` recorded before its staging refresh, so a map is ordered behind whatever context produced the texture's contents. The Windows CI lane builds the debug leaves before its best-effort leaf test step and selects WARP with software GL so `test_d3d11.c`'s Windows arm and `test_cross_library.c` execute there (TESTING.md § Per-language workflows).
...
- C headers state every platform-tied export's platform with a `Platforms:` line and a preamble section; the `edgefirst.tensor` and `edgefirst.image` stubs carry the same `Platforms:` sections. `ef_image_processor_convert_fence` is declared on every platform (`ENOTSUP` off Unix). `ImageProcessor::convert_with_fence` is no longer Unix-only.
- Under the dynamic backend (the Python wheels, and any consumer built with the `dynamic` feature) `cuda_map()` / `cuda_map_mut()` now reach a CUDA registration the library made for the tensor — D3D11 textures on Windows, DMA-BUF tensors on CUDA-capable Linux hosts through `try_init_dma_cuda` — where they answered `None` before. `TensorDyn::cuda()` still reports only an in-process registration, so consumers gate on the map returning `Some`, not on `cuda()`.
- The Python `convert`, `convert_deferred` and `convert_with_fence` bindings write the recorded GPU completion back onto the destination object the caller passed, so a foreign destination's `gpu_completion()` reflects the convert that just wrote it. The C API records on the tensor directly and needs no such step.
- Supersedes two 0.29.0 statements: `tests/gpu_policy.py` now requires a GPU-backed `create_image()` destination (a D3D11 texture, or a PBO) under `HAL_TEST_REQUIRE_GL=1`, and Windows GPU destinations are D3D11 textures rather than PBO transfers (`is_gpu_buffer_available()` is true on Windows).
...
- **Documentation review for 0.30.0.** Every claim below was checked against the code, not just re-read:
  - `CONTRIBUTING.md` said "Windows is compile-check only — CI runs `cargo check` there and nothing more" (twice). Windows has had three full lanes since 0.29.0 — `build-and-test-windows` (clippy, the Rust suite with coverage, the modular C-API tests, the wheel build and layout check, gpu-marked pytest on WARP), `test-windows-warp` (the image crate's GL tests on ANGLE D3D11 WARP), and `package-windows-capi` (release C-API build, package and smoke). The prerequisites, the ANGLE notes (which named only macOS/iOS, though Windows needs ANGLE just as much) and the CI section now describe what actually runs.
  - README's headline Features listed neither the D3D11 texture among the zero-copy backings nor Windows among the supported platforms, and scoped zero-copy CUDA to Jetson; the `edgefirst-tensor` row of the component table listed four of its seven memory backings.
  - README's internal dependency graph drew `edgefirst-image → edgefirst-decoder` as a required edge, and `crates/image/ARCHITECTURE.md` called it "unconditional". It is the `decode` feature and it is **off by default** — that decoupling is the point of 0.29.0's `Decoder.draw_onto`. Both now show the real edges, including the required `edgefirst-decoder-abi` and the correctly-gated `codec`, `tracker` and Linux-only `g2d-sys`.
  - `ARCHITECTURE.md`'s tracing field catalog described spans that are not emitted. `image.gl_init` fires only on macOS/iOS, Android and Windows — Linux brings its display up in `context.rs` and emits none — so the `platform` list no longer advertises `"linux"` and the `backend` list is the three import mechanisms that exist (`"iosurface"` / `"ahardwarebuffer"` / `"d3d11"`), not the per-convert transfer choice it was conflated with. `tensor.map` records `memory` in two nested spans, an outer `TensorMemory` variant from the generic `Tensor::map_impl` that every map has and an inner string from the platform-native backings; only the inner one had been described. `*_memory` listed a `Dma` variant that does not exist (`DmaBuf`), and omitted `IoSurface`, `Pbo` and `Cuda`. The span-name table gained `python.convert_with_fence` (added this release) alongside `convert_deferred` / `flush` / `tile_one`, and the fused `codec.decode_jpeg.write_*` family from 0.28.2. The source-tree listing was missing `tensor-abi/`, `tensor-ffi/`, `decoder-abi/` and `python-tracker/`.
  - **`tests/docs/test_span_catalog.py` now pins the tracing catalog to the source**, so this class of drift cannot return. Four directions: every span name the catalog lists is emitted somewhere; every quoted field value it documents is recorded by some span macro; every span the source emits appears in a crate's `ARCHITECTURE.md`; and the `*_memory` row lists exactly the `TensorMemory` variants. Writing it immediately found two more errors the manual review had missed — the catalog offered `image.convert.cpu` as the span identifying a CPU-serviced convert, but the CPU backend opens no span at that level (only `image.convert.cpu.*` sub-steps, so the row now says so), and `python.decode_proto` was emitted but documented nowhere. That second one had been invisible to hand-checking because its `trace_span!` puts the name on the following line, which an eyeball grep anchored to `!("` never sees. The field-value check deliberately searches span-macro arguments rather than the whole source: `"linux"` occurs in hundreds of `#[cfg(target_os = "linux")]` attributes, so a corpus-wide search would have accepted the exact stale value the check exists to reject.
  - Four dead documentation links to `crates/hal/`, `crates/python/` and `tensor_dyn.rs` — all removed or moved by 0.29.0's modular split — in the tracker, image and tensor architecture documents. Every relative and in-repo link in the 66 tracked markdown files now resolves.
  - `crates/tracker/README.md` showed two identical `edgefirst-decoder` dependency snippets under "or when depending on the decoder directly"; the first had been `edgefirst-hal` before the umbrella crate was deleted. Stale `0.28` / `0.29` pins in the decoder, tracker and egl READMEs now read `0.30`.
  - The `edgefirst.image` stub gained the `Platforms:` section on `convert_with_fence` that the C header has carried since this release's header pass.
- `ImageProcessor::draw_decoded_masks` and `draw_proto_masks` (and `set_class_colors`, which feeds them) reach the OpenGL backend on Windows, macOS, iOS and Android. Their GL arms were gated `target_os = "linux"` while `convert`'s were not, so those platforms drew on the CPU: a `create_image()` texture reported no `gpu_completion()` after a draw, and a draw into a read-only D3D11 texture failed because the CPU backend mapped it for writing. The GL draw path now records a completion on the destination the way a convert does (`GlPlatform::record_completion`), so a drawn texture carries a fence value; the Python `draw_proto_masks` publishes it onto the caller's object as `draw_decoded_masks` already did. The forced-OpenGL draw scenario test runs on every `opengl` platform. `ImageProcessor::set_class_colors` now reaches every available backend in the auto chain instead of returning at the first that accepts, and returns the first error: the palette is state a later draw reads, and which backend that draw lands on is not known when the colours are set, so a backend that never received them rendered the default palette silently. A forced backend still receives them alone.
- GL mask draws onto a zero-copy destination (a D3D11 texture, an IOSurface, a Linux dma-buf) with a host-memory background lost the background: the upload path swapped the host readback texture in as the colour attachment, so the frame never reached the destination. The background is now drawn into the destination framebuffer through the 2D source texture (zero-copy attach where the platform imports as `TEXTURE_2D`, upload otherwise), which also gives Windows and macOS a texture-backed background; the EGLImage blit stays on external-OES platforms. One case changes from silence to an error rather than to correct output: a `Bgra` background onto a zero-copy destination now returns `NotSupported`, because the 2D source draw has no `Bgra` arm and the CPU fallback renders only `Rgba`/`Rgb`, where before it returned `Ok` with the destination unwritten. A `Bgra` destination with no background is unaffected.
- A GL mask draw into a `view()`/`batch()` destination, or one carrying a `plane_offset`, is declined. This is the policy `convert` already applies: a sub-region destination is lowered to a band only on the u8/i8 zero-copy path, and only for a convert. Two of the draw's three arms could not place one. The zero-copy arm sets `glViewport(0, 0, w, h)` on the imported parent surface with none of the band lowering the convert path does, so the frame landed in the parent's first rows, and the PBO readback writes at buffer offset 0, which `plan_pbo_readback` already refused. The Mem readback could place one (a view's mapping starts at the view origin with the parent's pitch) and now goes to the CPU backend instead, so the draw carries one rule rather than a per-arm one. A forced-OpenGL draw into a view returns `NotSupported`.
...
- **Windows GPU acceleration.** The OpenGL ES engine now runs on Windows (x86_64) on [ANGLE](https://github.com/google/angle) over Direct3D 11, via a new `GlPlatform` leaf (`crates/image/src/gl/platform/windows.rs`). GPU destinations are PBO tensors (the desktop-Linux NVIDIA path); there is no zero-copy buffer kind on Windows yet, so `is_gpu_buffer_available()` stays `false` and D3D11 shared-texture tensors (and CUDA via D3D11 interop) are a follow-on. The loader searches `EDGEFIRST_ANGLE_PATH`, then next to the loading module (wheel `.pyd` / C archive DLL), then next to the executable, then the default DLL search path. `EDGEFIRST_ANGLE_ADAPTER` selects the D3D11 adapter (`hardware`, `warp`, `discrete`, a LUID, or a description substring) through DXGI enumeration; WARP is classified as a software renderer (`EDGEFIRST_ALLOW_SOFTWARE_GL=1` to opt in). Validated on an NVIDIA GeForce RTX 3070 (`ANGLE (NVIDIA, ... Direct3D11 ...)`).
