...
- C headers state every platform-tied export's platform with a `Platforms:` line and a preamble section; the Python stubs carry the same `Platforms:` sections. `ef_image_processor_convert_fence` is declared on every platform (`ENOTSUP` off Unix). `ImageProcessor::convert_with_fence` is no longer Unix-only.
