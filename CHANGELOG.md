...
- **105 mutants that survived the decoder's suite are now caught: lines every test executed but none asserted on.** The largest single gap was `XYWH::to_xyxy_float`, which the postprocess paths call and nothing tested — the only coverage went through `ndarray_to_xyxy_float`, a hand-inlined copy of the same arithmetic, so all twenty mutants in the real method survived. The class-aware and `_extra` NMS variants had `max_det` truncation tests but no suppression test, so a loop that stopped at the first survivor, suppressed across classes instead of within one, or marked a suppressed box with `1.0` instead of `-1.0` all went unnoticed. The score threshold is a floor and no test sat a score exactly on it. `Quantization::try_from` asserted only the error variant, which the catch-all arm produces too, so guard mutations that skipped the intended match arm still passed. `postprocess_modelpack_split_float` had no direct test at all. Of the sixty-three still surviving, none is a test gap: thirty-three are in `#[cfg(target_arch = "aarch64")]` code an x86_64 build never compiles, and the rest are equivalent mutants — `identity()` against a `Default` that is the same value, `survivors += 1` guarded by a `.take(cap)` that makes the early break unobservable, capacity arithmetic that only sizes a `Vec`, and layout dispatch whose two paths agree by design.
