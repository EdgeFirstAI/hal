...
| Tier | When | What |
|------|------|------|
| **Quick** | every non-draft PR push, and push to `main` | fmt (hard gate), clippy, nextest, ruff, dependency license — one Linux job, ~15 min budget |
...
| `sbom.yml` | Attach SBOM on a published GitHub Release only |
...
Callers currently pin `fd38981b2b94ae195801a21660e9ccc3fb222a4a` (pre-command hook, [EdgeFirstAI/.github#22](https://github.com/EdgeFirstAI/.github/pull/22)). Dependabot bumps both `uses:` and `shared-sha`.
